These instructions must be missing a step ortwo.  Or the product has a catastrophic bug in it  But when I follow these instructs and end up with spree deployed to render NOTHING WORKS.  All the URLS in the links aere wrong.  CSS files are not found.  So the layout a f-ed.  All the urls are pointing to localhost:3000.  But the app is on a render server port 443, ok?  If the point of this documentation is to entice people to use the spree, it's doing the opposite.
